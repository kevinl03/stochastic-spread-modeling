...
%%% ----- ACM metadata -----
\setcopyright{none}
\acmConference[ICAIF '26]{5th ACM International Conference on AI in Finance}{November 2026}{Brooklyn, NY, USA}

\begin{document}

\title{Cross-Exchange Statistical Arbitrage in Cryptocurrency Markets:\\Exploiting Venue-Specific Price Latency via Ornstein--Uhlenbeck Mean Reversion}
...
\begin{enumerate}
    ...
    \item We characterize \textbf{exchange-specific price latency}---Crypto.com lagging by up to 38~minutes for CRV and MEXC lagging by 1.5~minutes for WIF---as a persistent, exploitable microstructural feature.
    \item We compare OU-based signals against a rolling z-score baseline, finding that OU outperforms in the cross-exchange setting despite recent single-exchange findings to the contrary~\cite{suchato2024application}.
    \item We provide a transparent assessment of backtesting limitations and estimate realistic profit degradation under execution frictions.
\end{enumerate}
...
Our backtest uses one-minute close prices with instantaneous execution, zero slippage, and 100\% fill probability. These assumptions inflate reported profitability. Table~\ref{tab:realistic} estimates the impact of realistic execution frictions on the best-performing strategy (WIF, Binance--MEXC, OU).
...
We presented a cross-exchange statistical arbitrage framework for cryptocurrency markets that exploits venue-specific price latency using Ornstein--Uhlenbeck mean-reversion modeling. Our 30-day empirical study across 5~assets and 7~exchanges reveals:
...
\begin{enumerate}
    ...
    \item Stablecoin arbitrage is definitively unviable (fee-to-spread ratio of 25.9$\times$).
\end{enumerate}

Future work should validate these findings with live paper trading under realistic execution latency, extend the analysis to order-book-level data with bid-ask spread modeling, and investigate reinforcement learning for dynamic threshold optimization.
