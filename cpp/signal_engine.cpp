 ...
 *   1. OU parameter estimation (OLS-based, closed-form normal equations)
 *   2. Rolling z-score computation
 *   3. OU-based z-score computation
 *   4. Batch signal generation for backtesting
 *
 ...
 *
 * Instead of calling LAPACK lstsq, we solve the 2x2 normal equations
...
OUParams estimate_ou_params_impl(const double* data, size_t n, double dt) {
    ...
    // Single pass: accumulate sufficient statistics for 2x2 normal equations
 ...
 *
 * This eliminates the Python `for i in range(warmup, n)` loop entirely.
 * Each iteration does: signal computation + trade logic — all in native code.
 */
